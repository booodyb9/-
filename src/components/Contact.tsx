      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-3"><motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-3xl p-8 sm:p-12 shadow-[0_20px_60px_rgb(0,0,0,0.3)] text-[#0F172A] border border-gray-100">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><Field label="الاسم الكريم"><input required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="field" placeholder="أحمد محمد" /></Field><Field label="رقم الجوال"><input type="tel" required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} dir="ltr" className="field text-right" placeholder="+966 5X XXX XXXX" /></Field></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><Field label="البريد الإلكتروني (اختياري)"><input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="field" /></Field><Field label="نوع الخدمة المطلوبة"><select value={formData.service} onChange={(e) => setFormData({ ...formData, service: e.target.value })} className="field"><option value="">اختر الخدمة...</option><option value="واجهات زجاجية">واجهات زجاجية</option><option value="قواطع مكتبية">قواطع مكتبية</option><option value="أبواب زجاجية">أبواب زجاجية</option><option value="كبائن شاور">كبائن شاور</option><option value="مرايا">مرايا</option><option value="درابزين">درابزين</option><option value="أخرى">أخرى</option></select></Field></div>
            <Field label="تفاصيل الطلب"><textarea required value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={4} className="field resize-none" placeholder="اكتب تفاصيل مشروعك أو استفسارك هنا..." /></Field>
...
function Field({ label, children }: any) { return <div><label className="block text-sm font-bold text-gray-700 mb-2">{label}</label>{children}</div>; }
