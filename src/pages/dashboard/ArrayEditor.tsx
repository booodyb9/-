...
interface ArrayEditorProps {
  ...
  schema: { key: string; label: string; type: 'text' | 'textarea' | 'image' | 'number' | 'boolean'; }[];
  token?: string | null;
}
...
export default function ArrayEditor({ value, onChange, schema, token }: ArrayEditorProps) {
  ...
  const runAi = async (index: number, task: 'generate' | 'improve' | 'seo' | 'alt') => {
  ...
  return <div className="space-y-4">
    {items.map((item, index) => <div key={item.id || index} className="border border-gray-200 p-4 rounded-lg bg-gray-50 relative">
      ...
      <div className="mb-4 flex flex-wrap gap-2 bg-blue-50 p-3 rounded-lg border border-blue-100">
